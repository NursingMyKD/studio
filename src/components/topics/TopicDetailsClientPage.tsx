...
interface TopicDetailsClientPageProps {
  item: ContentItem;
}

export default function TopicDetailsClientPage({ item }: TopicDetailsClientPageProps) {
  const router = useRouter();
  const slug = item.slug;
  const [isClient, setIsClient] = useState(false);

  const { isBookmarked, toggleBookmark, isLoaded: bookmarksLoaded } = useBookmarks();
  // Note: The bookmark here is for the *main page* slug, not a specific section within details.
  ...
  if (!isClient) {
    // Basic skeleton or null while waiting for client-side hydration
    return (
      <div className="space-y-6">
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="pt-6">
            <div className="h-96 w-full bg-muted rounded animate-pulse"></div> {/* Skeleton for Markdown content */}
          </CardContent>
        </Card>
    ...
    <div className="space-y-6">
      ...
      <div className="mt-8 text-center">
        ...
        <Button variant="outline" asChild>
          <Link href={`/topics/${slug}`} legacyBehavior>
            <>
              <ArrowLeft className="mr-2 h-4 w-4" /> Back to Overview
            </>
          </Link>
